...
function TableStockItems() {
  ...
  const getData = async () => {
    ...
    try {
      const res = await axios
        .get(
          `${process.env.REACT_APP_DOMAIN_API}` + "/StockItemReps",
          GetConfig()
        )
        .then(function (response) {
          ...
          if (response.status == 200) {
            ...
            Object.keys(resdata).forEach(function (key) {
              newres.push({
                id: key,
                inventory: resdata[key].InventoryID,
                description: resdata[key].Description,
                itemStatus: resdata[key].ItemStatus,
                itemClass: resdata[key].ItemClass,
                postingClass: resdata[key].PostingClass,
                lastSync: resdata[key].LastSync,
              });
            });
            setData(newres);
          }
  ...
  const Synch = async () => {
    ...
    try {
      await axios
        .post(
          `${process.env.REACT_APP_DOMAIN_API}` + "/StockItemSyncReps/Sync",
          {},
          GetConfig()
        )
        .then(function (response) {
          ...
          <div style={{ width: "100%", padding: "10px" }}>
            <DataGrid
              ...
              onCellClick={(params, event) => {
                ...
                history(`/master-data/stock-item/${params.row["inventory"]}`);
              }}
            />
          </div>
        </Paper>
