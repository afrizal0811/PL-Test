...
export default function BPOMPopup(props) {
  //state Customer ID
  ...
  const getData = async (page) => {
    ...
    try {
      const res = await axios
        .get(
          props.api +
            `page=${page}&rowsCount=${pageSize}&userId=${getEmployee()}${
              searchText.length > 0 ? "&noBPOM=" + searchText : ""
            }&BranchID=${getBrach()}`,
        ...
        <DialogActions>
          ...
          <Button
            onClick={() => {
              ...
              //   `/allocated-kuota/detail/${SelectedCust.RefNbr}`
              // );
            }}
            color="primary"
          >
